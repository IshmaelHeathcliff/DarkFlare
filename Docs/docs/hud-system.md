...
HUD 右上角状态条与菜单“状态”入口已接入：按定义显示图标、持有层数与下次到期时间，支持玩家 / 怪物选择、焦点浮窗及手柄滚动 / 返回。显示 / 可隐藏 / 从不显示三档由状态定义配置，可隐藏默认显示，共用开关只过滤界面。保持 Gameplay 与 UI 导航隔离，资源和刷新任务归属 Session，详见[状态生命周期与查看](./status-lifecycle-ui.md)。
