...
初版阶段已经接入的 SpriteSheet 继续作为历史运行时资产保留，直到独立迁移计划完成引用替换。本规范不要求立即拆分既有 Sheet，但既有 Sheet、人工 Sprite Editor Rect 和 sub-sprite fileID 不再作为新资产的生产模板。
...
| --- | --- | --- | --- |
| 玩家 / 人形 NPC 单帧 | 96×96 | 宽 52–66、高 86–90 px | `x = 48 ± 2 px`，底部边距 3–6 px |
| 普通人形怪物单帧 | 96×96 | 宽 76–84、高 86–90 px | `x = 48 ± 2 px`，底部边距 3–6 px |
| 迅捷四足怪物单帧 | 96×96 | 宽 68–78、高 48–56 px | 共享接地线，底部边距 4–8 px |
| 重装怪物单帧 | 96×96 | 宽 70–80、高 82–88 px | `x = 48 ± 2 px`，底部边距 3–6 px |
| 装备图标 | 96×96 | 最长边 72–80 px | Alpha 加权中心偏差不超过 3 px，任一边距不少于 6 px |
| 投射物 / 小型特效 | 64×64 | 由具体家族合同定义 | 默认画布中心；方向性拖尾必须保留安全边距 |
| 固定 UI 图标 | 64×64 | 由具体图标家族 Pilot 确定 | Alpha 加权中心与光学中心同时验收 |
| 地表纹理 | 512×512 | 允许全画布覆盖 | Center，四边无接缝 |
| UI 面板 / 状态 | 每个组件单独冻结 | 不使用通用缩放到框 | Slice 保护区和独立装饰区必须预先定义 |
| 世界物件 | 每个同类家族单独冻结 | 以玩家体量和网格用途为参照 | 接地点、占地和遮挡关系必须预先定义 |
...
Assets/Art/SpriteSheets/Phase5/  # 仅保留初版历史资产，不新增
```
