...
alpha 0.1.0 将三套物品页面合并为共享工作台：四槽装备位于共享玩家列上方，10×6 背包位于下方；该列在商店和打造中最大占内容区 49%，右侧上下文获得主要空间。商店使用更大的固定 10×6 商人背包且不显示滚动条，玩家与商人格内只保留图标。装备区保留两个非交互扩展位置，但不提前增加领域槽位。运行时只有一个顶层 `ItemTooltipView`，使用固定尺寸和固定顶部停靠，拖放后等待下一次主动预览再显示；整个浮窗子树输入穿透。面板与槽位使用统一 1 px USS 细边框，不再缩放旧粗 Frame。

alpha 0.1.0 的 HUD 移除武器卡片，保留生命和金币信息，并在左下角增加“当前属性”窗口。
`GetHudSnapshotQuery` 直接读取 `CombatActor.Stats` 的最终有效值，展示护甲、闪避、移动速度、暴击率以及
火焰、冰霜、闪电、混沌抗性；生命仍由独立生命条表达，避免重复。抗性显示与伤害结算一致，限制在
`-100%` 至 `75%`，换装后由装备事件触发重新查询。

| Controller | 只读数据入口 | 写入入口 | 主要刷新来源 |
| --- | --- | --- | --- |
| `HudController` | `GetHudSnapshotQuery` | 无 | Actor、生命、金币、最终有效属性、装备事件 |
| `InventoryPanelController` | `GetInventorySnapshotQuery` | 背包移动、精确装备 / 卸下与戒指交换 Commands | 背包、装备事件 |
