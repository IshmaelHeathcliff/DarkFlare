...
状态阶段 3 增加显式命中附加异常及七项异常抗性；来源参数在攻击发出时冻结，目标异常抗性在施加时读取。周期不递归附加异常，类型防御继续逐跳结算，见[异常与来源](./status-ailments.md)。
...
局部词条和全局词条在收集阶段分开。当前 `AttackSnapshotFactory` 把 Weapon 槽的 `LocalItem` 加入攻击快照，与 `GlobalActor` / `Skill` 共走 `DamageCalculator`；尚未实现独立的武器本地预结算乘区。因此匹配同一伤害包的局部与全局 Increase 当前相加。完整时序见[实际作用流程](./architecture/equipment-affix-damage/README.md)。
...
1. 根据技能 DamageSource 选择武器或技能基础伤害，并掷出基础伤害包。
2. 发射时冻结来源标签、攻击者有效属性、修改器及命中 / 暴击随机值；武器 LocalItem 在此加入修改器集合。
3. 碰撞时检查目标，读取当前防御者属性和标签，计算命中 / 闪避及暴击结论；未命中不进入来源伤害计算。
4. 执行单轮转换，同一来源包的总转换比例封顶 100%。
5. 对转换后的包执行额外获得；本轮新增包不递归参与额外获得。
6. 对每个包按标签与 StatId 匹配来源 Flat、Increase、More，依次应用固定值、加算增伤和独立乘区。
7. 命中判定为暴击时，应用暴击伤害倍率。
8. 逐包应用防御者 TargetTaken；当前 Increase 和 More 均逐项相乘，然后按最终伤害类型合并。
9. 物理命中使用护甲，元素与混沌使用对应抗性；裁剪最终值不小于零。
10. CombatSystem 提交生命变化并发送结算、实际受伤、死亡事件；存活且命中的目标可接受快照中的附加状态。

当前已实现命中、闪避和暴击；本流程没有实现格挡、穿透或护盾承伤。周期伤害使用独立入口，见本文末尾。
...
状态周期与异常施加已接入，见本文末尾及[状态战斗接入](./status-combat.md)。状态自动计时、存档和图标 UI 仍属后续阶段。`Temporary` Scope 本身仍不提供计时 / 叠层行为；实际效果复用 GlobalActor / Skill / TargetTaken，由状态模块管理寿命和来源。
