...
| 暴击 | `critical_chance` | 暴击率 | 已登记；当前命中上下文直接提供暴击结果，尚未自动掷骰 |
| 暴击 | `critical_damage` | 暴击伤害 | 暴击时作为额外伤害百分比；`50` 表示最终 `150%` |
| 命中 | `accuracy` | 命中值 | 已登记；当前默认命中，命中公式尚未接入 |
| 防御 | `armor` | 护甲 | 降低物理命中伤害 |
| 防御 | `evasion` | 闪避值 | 已登记；闪避公式尚未接入 |
...
当前全量 EditMode 验证为 75/75 通过，其中属性专项覆盖完整资产集合、暴击伤害百分比语义和非法配置诊断。
