...
`DarkFlare` 已完成首版单场景最小循环、初步体验优化、`alpha 0.1` 封板，以及 `alpha 0.2.0–0.2.2` 应用生命周期、稳定身份、内容目录、迁移和本地存档闭环。现有基线可以在 `Main.unity` 中完成战斗、掉落、拾取、四槽装备、交易、打造和返回战斗的完整循环，并能保存 `auto` 槽位、停止并精确恢复当前玩法 Session。下一阶段为 `alpha 0.2.3` 用户设置与本地化。
...
- `Assets/Scripts/Tests/EditMode/`：`DarkFlare.Tests.EditMode`，`alpha 0.2.2` 完成基线全量 `310/310` 通过。
...
生命周期的职责、状态、事务和禁止事项见[应用生命周期与会话作用域](./infrastructure/application-lifecycle.md)；内容、实例身份、DTO 与迁移规则见[稳定身份、内容目录与迁移框架](./infrastructure/content-identity-migration.md)；文件格式、代际存储、保存和 Restore 流程见[本地存档与 Session 恢复](./infrastructure/local-save.md)。
...
- `GameMenuController` 管理共享遮罩、三页签、存档 / 继续 / 新游戏入口、关闭和输入模式。
...
完整流程与模块边界见 [`gameplay-loop.md`](./gameplay-loop.md)，生命周期见[应用生命周期与会话作用域](./infrastructure/application-lifecycle.md)，稳定身份和迁移见[稳定身份、内容目录与迁移框架](./infrastructure/content-identity-migration.md)，本地持久化见[本地存档与 Session 恢复](./infrastructure/local-save.md)，输入、菜单和场景交互结构见 [`input-ui-system.md`](./input-ui-system.md)。
