...
稳定本地 ID 均使用小写 `snake_case`，持久化时由类型命名空间组成 `<namespace>:<local_id>` 的 `ContentId`；唯一正式目录 `core` v3 收录当前 106 个配置。目录、缺失策略和扩展规则见[稳定身份、内容目录与迁移框架](./infrastructure/content-identity-migration.md)。运行时随机规则继续沿用独立通道：怪物生命倍率 `0.85–1.15`、初始大剑伤害 `20–40`，掉落概率由各怪物掉落表独立配置。空手武器技能没有伤害回退。
...
| --- | --- | ---: | ---: | --- | --- | ---: | ---: |
| 荒原游魂 | `wasteland_wraith` | 52 | 2.6 | `6–10` | `0.75 / 0.75s` | 50% | 55 |
| 裂爪猎犬 | `razor_hound` | 36 | 3.6 | `4–7` | `0.75 / 0.55s` | 60% | 30 |
| 铁壳尸傀 | `iron_husk` | 96 | 1.7 | `10–14` | `0.75 / 1.0s` | 80% | 15 |
...
三者使用独立 `MonsterDefinition`、掉落表、Addressable Prefab、Sprite 和 Animator Controller，并保持统一的移动、攻击、受伤与死亡参数契约。移动参数统一为停止距离 `0.6`、软分离半径 `0.8`、软分离权重 `0.65`；接触攻击按表中的独立间隔执行，没有玩家全局受伤冷却。三张掉落表都覆盖二十件装备、金币与锻造矿石。每条装备权重 100，金币权重 2000（8–24 个），矿石权重 500（1–3 个）。装备合计权重 2000；装备生成仍使用对应稀有度和 25 个物品词条池。数量、分类及堆叠规则见[统一物品系统](./item-system.md)。
