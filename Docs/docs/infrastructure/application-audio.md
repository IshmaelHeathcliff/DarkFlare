...
`ApplicationHost` 在 Application 作用域内创建唯一 `AudioService`。场景和 Session 只能借用服务，不得创建 `AudioSource`、直接访问 `AudioMixer` 或释放 Application 资产。服务拥有 AudioListener 仲裁、Source 池、Addressables Clip 句柄、活动播放和待加载请求，并在 Application Shutdown 统一回收。

启动时先加载地址 `infrastructure/audio` 的 `AudioServiceConfiguration`，完成 Mixer、分类组、参数和 Cue 校验后才允许播放。配置加载句柄由宿主精确释放；Cue 的 Clip 句柄由专用 `AddressableAudioClipLoader` 精确释放。
...
| `Assets/Audio/AudioServiceConfiguration.asset` | `infrastructure/audio` | Mixer 组、参数名和 Cue 清单 |
...
- `AudioPlaybackHandle` 精确对应一次播放；重复 `Stop` / `Dispose` 返回已完成，不得二次释放 Source 或 Addressables 句柄。
...
当前只接入一个 UI Cue，不代表完整音乐或战斗音频内容已经制作。通用 Addressables 分组、标签、跨加载器治理和资源泄漏总审计属于 `alpha 0.2.6`。
