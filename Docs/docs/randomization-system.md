...
统一随机种子链覆盖怪物生成位置、怪物实例生命与词条、玩家攻击、怪物攻击、掉落和打造。`alpha 0.1.5` 在既有 `MonsterInstance` 顶层通道内增加实例子流；`alpha 0.1.6` 增加独立 `Crafting` 通道，不推进其他顶层随机序列。
...
| `Crafting` | 打造操作的数量、分布、目标、定义和数值子种子 |
...
`DiscardItemCommand` 将现有背包实例转移到世界，不重新生成物品、不推进 `Loot` 随机通道。先验证当前 Session、存活玩家、资源与可达落点，再创建并登记空拾取物；准备完成后移出背包，为同一实例分配 `WorldDropId`。准备失败保留原格，提交后才通知背包变化。

落点避开玩家所有有效碰撞 / 交互范围，并在场景矩形边界内按确定性方向寻找；沿途实体阻挡则换候选，没有合法位置时拒绝丢弃。边界使用当前封闭 `EdgeCollider2D` 的包围范围，不能把边界线的 `OverlapPoint` 当成内部区域判断。

拾取成功先清除拾取物中的实例、禁用并释放 Session 对象，再发布背包事件；重复碰撞和事件回调重入无法重复获得物品。保存继续保留世界 ID、物品 ID、种子和词缀，恢复后也不会因丢弃位置重叠而自动拾回。对应长期回归合并在 `SaveCaptureRestorePlayModeTests` 的完整快照恢复流程。
