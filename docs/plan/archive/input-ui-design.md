...
> 归档状态：已完成，归档于 2026-07-20。本文保留 8a–8f 的执行过程；当前实现见 [输入与运行时 UI](../../input-ui-system.md)。

本文档记录玩家输入层重构与 UIToolkit UI 架构，是最小循环第 8 步“用 `Main.unity` 串成一轮完整循环”的执行记录。8a–8f 均已完成。
