...
- 使用 UTF-8 编码
- 除游戏设计文档外，所有文档都放在 `docs/` 目录下。
- 要区分计划文档与模块功能文档，计划文档放在`docs/plan`下
- 计划完成时，应该总结模块文档，然后将原本的计划文档归档到 `docs/plan/archive`
- 文档应分模块管理，可创建子目录管理，单个文档不应该过长。
- 修改代码前先阅读相关文档。
- 如果文档与代码冲突，以代码现状为准，并同步修正文档。
- 新功能完成后，主动检查是否需要更新文档 。
- 不要让文档长期落后于实现。
- `design/` 目录仅存放游戏设计文档。只有在纯游戏设计任务中显式调用 `/design`（`$design`）Skill 时，才允许读取或修改其中的文档。
- 开发、实现、调试、测试等任务禁止读取、引用或修改 `design/`；同时包含设计与开发的请求必须先拆分任务。
